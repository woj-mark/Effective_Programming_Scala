...
object Firework:
   ...
   * by calling the operation `next` on the underlying state typ;
...
case class Launched(countDown: Int, position: Point, direction: Angle, numberOfParticles: Int, particlesColor: Color) extends Firework:
  ...
  def next: Firework =
    if countDown > 0 then
      val newLaunchedPosition =  Motion.movePoint(position,direction,Settings.propulsionSpeed)
      val newCountDown = countDown - 1
      Launched(newCountDown,newLaunchedPosition,direction,numberOfParticles, particlesColor)
